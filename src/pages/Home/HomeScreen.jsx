...
function HomeScreen() {
    const { key, getTrendingVideo } = useMoviesStore()
    const [imgLoading, setImgLoading] = useState(false)
    const [trendingVideo, setTrendingVideo] = useState({})

    useEffect(() => {
        const fetchTrendingVideo = async () => {
            const result = await getTrendingVideo()
            if (result?.success) {
                setTrendingVideo(result.movie)
            }
        }
        fetchTrendingVideo()
    }, [key])
    ...
    return (
        <>
            <div className='relative h-screen text-white '>
                ...
                <img
                    src={`https://image.tmdb.org/t/p/original${trendingVideo.backdrop_path}`}
                    alt='Hero img'
                    className='absolute top-0 left-0 w-full h-full object-cover'
                />

                <div className='absolute top-0 left-0 w-full h-full bg-black/50' aria-hidden='true' />
                ...
                <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
                    ...
                    <div className='max-w-2xl'>
                        ...
                        <p className='mt-2 text-lg'>
                            {trendingVideo?.release_date?.substr(0, 4) || trendingVideo?.first_air_date?.substr(0, 4)} | {trendingVideo?.adult ? '18+' : 'PG-13'}
                        </p>
